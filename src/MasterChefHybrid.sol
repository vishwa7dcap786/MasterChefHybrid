...
interface sushiToken is IERC20{
...
contract MasterChefHybrid is BoringOwnable, BoringBatchable {
    ...
    function pendingSushi(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 sushiReward = multiplier.mul(sushiPerBlock).mul(pool.allocPoint)/totalAllocPoint;
            accSushiPerShare = accSushiPerShare.add(sushiReward.mul(ACC_SUSHI_PRECISION)/lpSupply);
        }
        return int256(user.amount.mul(accSushiPerShare) / ACC_SUSHI_PRECISION).sub(user.rewardDebt).toUInt256();
    }
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0) {
                ...
                sushi.authorizedMint(devaddr, sushiReward/10);
                sushi.authorizedMint(address(this), sushiReward);
            }
            pool.lastRewardBlock = block.number.to64();
            poolInfo[_pid] = pool;
            emit LogUpdatePool(_pid, pool.lastRewardBlock, lpSupply, pool.accSushiPerShare);
        }
    }
